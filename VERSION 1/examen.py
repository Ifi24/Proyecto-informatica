...
class GraphApp:
    def __init__(self, root):
        ...
        tk.Button(left_frame, text="Mostrar nuestro", width=20, command=self.show_example_graph).pack(pady=10)
        ...
        # Reachability
        self.nodetoreach_entry = tk.Entry(left_frame)
        self.nodetoreach_entry.pack()
        self.nodetoreach_entry.insert(0, "Nodo para encontrar caminos")

        tk.Button(left_frame, text="Encontrar todos los caminos posibles", command=self.show_reachability).pack(pady=10)
        ...
        # examen
        self.nodetofind_entry = tk.Entry(left_frame)
        self.nodetofind_entry.pack()
        self.nodetofind_entry.insert(0, "Texto")

        tk.Button(left_frame, text="Encontrar nodos con nombre", command=self.buscar_nombre).pack(pady=10)
    ...
    def shortest_path(self):    #encuentra el camino más corto entre nodos
        ...
        if path:
            ...
            for i in range(len(path.nodes) - 1):
                n1 = path.nodes[i]
                n2 = path.nodes[i + 1]
                self.ax.plot([n1.x, n2.x], [n1.y, n2.y], 'g-', linewidth=5)  
    ...
    def show_reachability(self):
        ...
        start_name = self.nodetoreach_entry.get().strip()
        start_node = self.graph.GetNodeByName(start_name)

        if not start_node:
            messagebox.showinfo("No encontrado", f"No se encontró el nodo '{start_name}'.")
            return

        return self        

    def buscar_nombre(self):
        nombre = self.nodetofind_entry.get().strip()
        nodos=self.graph.nodes
        nodosconnombre=[]

        for node in nodos:
            if nombre in node.name:
                nodosconnombre.append(node.name)
        if nodosconnombre:
            self.output_text.insert (tk.END,  f'Los nodos con el nombre son: {nodosconnombre} \n')
        else:
            self.output_text.insert (tk.END,  f'No se han encontrado nodos con el nombre {nombre} \n')
